...
import { Session, ConversationElement, ConversationContext, ConversationContentText, ConversationContentImage, ConversationSegment } from "./../app/types";
...
export class Memory {
        ...
        this.session.conversation_history.push(
            {
                ...
                current_context: this.concatConversationText(this.session.conversation_context)
    ...
    accumulateAIStream(textDelta): void {
        if (this.verbose) {
    ...
    concatConversationText(context: ConversationContext): string {
        return context.context.map(segment => {
            // Concatenate the content for each segment
            const contentText = segment.content.map(content => {
